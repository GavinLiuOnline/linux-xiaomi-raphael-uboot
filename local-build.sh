...
interactive_select() {
    ...
    if [ "$OS_FAMILY" = "debian" ]; then
        ...
        read -r -p "  请输入 [1/3] (默认: 1): " ubu_ver_choice
        case "${ubu_ver_choice:-1}" in
    ...
    read -r -p "  请输入 [1/3] (默认: 3): " type_choice
    ...
    echo "  1) 7.0   (主线最新版)"
    echo "  2) 6.18  (稳定版)"
    read -r -p "  请输入 [1/2] (默认: 2): " kern_choice
    case "${kern_choice:-2}" in
